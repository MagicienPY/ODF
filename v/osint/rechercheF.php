            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="overview-wrap">
                        <h2 class="title-1">ODF-OSINT Dashboard/Facebook</h2>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST['search'])) {
                        ...
                        // Requête initiale pour obtenir un request_id
                        $url = "https://api.social-searcher.com/facebook-search/v2/search?q=" . urlencode($searchQuery) . "&network=twitter&limit=10&key=$API_KEY";
                        $ch = curl_init();
                        ...
                        if (curl_errno($ch)) {
                            ...
                            if (isset($data['meta']['requestid'])) {
                                ...
                                // Attendre 60 secondes pour permettre à l'API de traiter la recherche
                                sleep(60);
                                    ...
                                    if (curl_errno($ch)) {
                                        ...
                                        if ($status == 'finished') {
                                            ...
                                            foreach ($fetch_data['posts'] as $post) {
                                                ...
                                                echo "<small>Publié le : " . htmlspecialchars($post['created']) . "</small>";
                                                echo "</div>";
                                            }
                                            echo "</div>";
                                            break;
                                        } elseif ($status == 'pending') {
                                            // Attendre 5 minutes avant de refaire une tentative
                                            sleep(300);  
                                        }
                                    }
                                    $attempt++;
                                ...
                                if ($status == 'finished') {
                                    ...
                                    foreach ($fetch_data['posts'] as $post) {
                                        echo "<div class='post-item'>";  // div pour chaque publication
                                        echo "<p><strong>Utilisateur :</strong> " . htmlspecialchars($post['user']['name'] ?? 'Utilisateur inconnu') . "</p>";
                                        echo "<p>" . htmlspecialchars($post['text']) . "</p>";
                                        echo "<small><strong>Publié le :</strong> " . htmlspecialchars($post['created']) . "</small>";
                                        echo "</div><hr>"; // Séparation entre chaque publication
                                    }
